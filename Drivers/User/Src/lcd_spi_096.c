/***
   ...
   *  @description
	*
	*	实验平台：反客STM32H7B0VBT6核心板 （型号：FK7B0M1-VBT6）+ 0.96寸液晶模块（型号：SPI096M1-160*80） 
...
volatile uint16_t LCD_Width   = 0;
volatile uint16_t LCD_Height  = 0;
...
void MX_SPI4_Init(void)
{
	...
	if( lcd_panle == panle_096 )
	{
	// SPI的内核时钟设置为120M，再经过8分频得到 15M 的SCK驱动时钟 （屏幕控制器ST7735所允许的最高速度）
...
void SPI_LCD_Init(void)
{
 ...
 // 打开显示指令，LCD控制器在刚上电、复位时，会自动关闭显示 
   ...
   switch (lcd_panle)
   {
      case panle_096:   LCD_Width = 80;   LCD_Height = 160;LCD_SetDirection(Direction_H_Flip); break;
      case panle_114:   LCD_Width = 135;  LCD_Height = 240;LCD_SetDirection(Direction_H_Flip); break;
      case panle_130:   LCD_Width = 240;  LCD_Height = 240;LCD_SetDirection(Direction_V); break;
      case panle_169:   LCD_Width = 240;  LCD_Height = 280;LCD_SetDirection(Direction_V); break;    
      case panle_200:   LCD_Width = 240;  LCD_Height = 320;LCD_SetDirection(Direction_V);break;   		
      default:break;  
   }
...
void LCD_SetDirection(uint8_t direction)
{
   ...
   {     		         
      ...
      switch (lcd_panle)   // 设置控制器坐标偏移量
      {
         case panle_096:  LCD_WriteData_8bit(0xA8);LCD.X_Offset = 1;    LCD.Y_Offset  = 26;   break;
         case panle_114:  LCD_WriteData_8bit(0x70);LCD.X_Offset = 40;   LCD.Y_Offset  = 53;   break;
         case panle_130:  LCD_WriteData_8bit(0x70);LCD.X_Offset = 0;    LCD.Y_Offset  = 0;   break;
         case panle_169:  LCD_WriteData_8bit(0x70);LCD.X_Offset = 20;   LCD.Y_Offset  = 0;   break;
         case panle_200:  LCD_WriteData_8bit(0x70);LCD.X_Offset = 0;   	LCD.Y_Offset  = 0;   break;			
         default:break;  
      }
   }
   else if( direction == Direction_V )
   {
      ...
      switch (lcd_panle)   // 设置控制器坐标偏移量
      {
         case panle_096:  LCD_WriteData_8bit(0xC8);LCD.X_Offset = 26;    LCD.Y_Offset  = 1;   break;
         case panle_114:  LCD_WriteData_8bit(0x00);LCD.X_Offset = 52;   LCD.Y_Offset  = 40;   break;
         case panle_130:  LCD_WriteData_8bit(0x00);LCD.X_Offset = 0;    LCD.Y_Offset  = 0;   break;
         case panle_169:  LCD_WriteData_8bit(0x00);LCD.X_Offset = 0;   LCD.Y_Offset  = 20;   break;
         case panle_200:  LCD_WriteData_8bit(0x00);LCD.X_Offset = 0;   	LCD.Y_Offset  = 0;   break;				
         default:break;  
      }
   }
   else if( direction == Direction_H_Flip )
   {
      ...
      switch (lcd_panle)   // 设置控制器坐标偏移量
      {
         case panle_096:  LCD_WriteData_8bit(0x78);LCD.X_Offset = 1;    LCD.Y_Offset  = 26;   break;
         case panle_114:  LCD_WriteData_8bit(0xa0);LCD.X_Offset = 40;   LCD.Y_Offset  = 52;   break;
         case panle_130:  LCD_WriteData_8bit(0xa0);LCD.X_Offset = 80;    LCD.Y_Offset  = 0;   break;
         case panle_169:  LCD_WriteData_8bit(0xa0);LCD.X_Offset = 20;   LCD.Y_Offset  = 0;   break;
         case panle_200:  LCD_WriteData_8bit(0xa0);LCD.X_Offset = 0;   	LCD.Y_Offset  = 0;   break;				
         default:break;  
      }
   }
   else if( direction == Direction_V_Flip )
   {
      ...
      switch (lcd_panle)   // 设置控制器坐标偏移量
      {
         case panle_096:  LCD_WriteData_8bit(0x08);LCD.X_Offset = 26;    LCD.Y_Offset  = 1;   break;
         case panle_114:  LCD_WriteData_8bit(0xc0);LCD.X_Offset = 53;   LCD.Y_Offset  = 40;   break;
         case panle_130:  LCD_WriteData_8bit(0xc0);LCD.X_Offset = 0;    LCD.Y_Offset  = 80;   break;
         case panle_169:  LCD_WriteData_8bit(0xc0);LCD.X_Offset = 0;   LCD.Y_Offset  = 20;   break;
         case panle_200:  LCD_WriteData_8bit(0xc0);LCD.X_Offset = 0;   	LCD.Y_Offset  = 0;   break;					
         default:break;  
      }      						
   }   
}
...
void LCD_DrawLine_fV(uint16_t x, uint16_t y, uint16_t height)
{
   uint16_t i ; // 计数变量
   ...
   LCD_SetAddress( x, y+height-1,x, y);	     // 设置坐标	
...
void LCD_DrawLine_fH(uint16_t x, uint16_t y, uint16_t width)
{
   uint16_t i ; // 计数变量
   ...
   LCD_SetAddress( x+width-1, y,x, y);	     // 设置坐标	
