...
#define SMBUS_PORT	GPIOD      //PB端口(端口和下面的两个针脚可自定义)
#define SMBUS_SCK		GPIO_PIN_0 //PD0：SCL
#define SMBUS_SDA		GPIO_PIN_1 //PD1：SDA
...
//IO方向设置
#define SDA_IN()  {GPIOD->MODER&=~(3<<(1*2));GPIOD->MODER|=0<<1*2;}	//PH5输入模式
#define SDA_OUT() {GPIOD->MODER&=~(3<<(1*2));GPIOD->MODER|=1<<1*2;} //PH5输出模式
...
void SMBus_Init()
{
    GPIO_InitTypeDef    GPIO_InitStruct;
		...
		GPIO_InitStruct.Pin 			= SMBUS_SCK | SMBUS_SDA;;				   // SCL引脚
...
//		GPIO_InitStruct.Pin 			= SMBUS_SDA;				// SDA引脚
//		HAL_GPIO_Init(SMBUS_PORT, &GPIO_InitStruct);		
